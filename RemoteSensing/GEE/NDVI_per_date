...
var NDVIpalette = [
  'FFFFFF', // White for very low NDVI
  'A7D080', // Light green
  '70C25F', // Medium green
  '38A700', // Darker green
  '006400'  // Dark green for high NDVI
];
...
// Calculamos el NDVI y lo añadimos
function calculateNDVI(img) {
    var ndvi = img.normalizedDifference(['B8', 'B4']).rename('NDVI');
    return img.addBands(ndvi)}
var s2_ndvi= s2.map(calculateNDVI)
...
var s2_ndviWithMasks = s2_ndvi.linkCollection(csPlus, [QA_BAND]).map(generateMasks);
print(s2_ndviWithMasks)

// Add `date` and `datatake_id` properties
var s2_ndviWithDate = s2_ndviWithMasks.map(function(image) {
...
print('s2_ndviWithDate:', s2_ndviWithDate);
...
var combinedMasks = ee.ImageCollection(
    ee.List(
        s2_ndviWithDate.aggregate_array('date').distinct().map(function(date) {
            // Filter images for the current date
            var imagesForDate = s2_ndviWithDate.filter(ee.Filter.eq('date', date));
...
// Add the masked and clipped NDVI
...
uniqueDates.evaluate(function(dates) {
    dates.forEach(function(timestamp) {
        var daily_NDVI_Clipped = Masked_combinedMasks.filter(ee.Filter.eq('system:time_start', timestamp)).first();
        Map.addLayer(daily_NDVI_Clipped.clip(ROI).select('NDVI'),
        NDVIpalette, 'Masked NDVI' + ee.Date(timestamp).format('YYYY-MM-dd').getInfo());
    });
});

// Calculate average NDVi for image
var ndviPerDate = Masked_combinedMasks.map(function(image) {
    var meanNDVI = image.select('NDVI')
        .reduceRegion({
            reducer: ee.Reducer.mean(), // Mean NDVI for all pixels in the ROI
            geometry: ROI,
            scale: 10, // Sentinel-2 resolution
            maxPixels: 1e9
        }).get('NDVI'); // Extract NDVI value
    var pixelCount=image.select('NDVI')
      .reduceRegion({
            reducer: ee.Reducer.count(),
            geometry: ROI,
            scale: 10,
            maxPixels: 1e9
        }).get('NDVI'); // Pixel count for the NDVI band
...
// Convert NDVI results to a feature collection
var ndviPerDateCollection = ee.FeatureCollection(ndviPerDate);

// Export results as CSV
Export.table.toDrive({
    collection: ndviPerDateCollection,
    description: 'NDVI_Per_Date2018',
    folder: 'NDVI_EarthEngine_Exports',
    fileFormat: 'CSV'
});
